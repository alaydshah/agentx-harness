...
class PhaseRunner(TaskManagerMixin):
    ...
    def _generation_drain_snapshot(self) -> dict[str, int | bool]:
        """Snapshot every ledger that can still create Agentic Replay work."""
        counter = self._progress.counter
        tree_open = (
        ...
        return {
            "roots_closed": counter.root_admission_closed,
            "root_turns_pending": max(
                0, counter.total_session_turns - counter.root_requests_sent
            ),
            "execution_running": execution_running,
            "in_flight": counter.in_flight,
            "tree_open": tree_open,
            "tree_reserved": tree_reserved,
            "scheduler_pending": self._scheduler.pending_count,
            "scheduler_running": self._scheduler.running_count,
            "branch_pending": branch_pending,
            "replay_pending": replay_pending,
            "generation_failed": self._scheduler_failure is not None,
        }

    def _is_generation_complete(self) -> bool:
        """Whether root admission is closed and no future request is possible."""
        state = self._generation_drain_snapshot()
        return (
            state["roots_closed"] is True
            and state["root_turns_pending"] == 0
            and state["execution_running"] is False
            and state["in_flight"] == 0
            and state["tree_open"] == 0
            and state["tree_reserved"] == 0
            and state["scheduler_pending"] == 0
            and state["scheduler_running"] == 0
            and state["branch_pending"] is False
            and state["replay_pending"] is False
            and state["generation_failed"] is False
        )
    ...
    async def _wait_for_generation_drain(self, timeout: float | None) -> bool:
        ...
        Returns True on timeout. Polling is intentionally low-frequency: all
        ledgers are event-loop-local, and the 50 ms completion precision has no
        bearing on request timestamps or throughput.
        """

        async def wait_until_quiescent() -> None:
            next_report = time.monotonic() + 30.0
            stall_details_reported = False
            while not self._is_generation_complete():
                ...
                if now >= next_report:
                    state = self._generation_drain_snapshot()
                    self.info(
                        "Agentic generation drain waiting: "
                        + " ".join(f"{key}={value}" for key, value in state.items())
                    )
                    if (
                        not stall_details_reported
                        and state["in_flight"] == 0
                        and state["scheduler_pending"] == 0
                        and state["scheduler_running"] == 0
                        and (
                            state["branch_pending"] is True
                            or state["replay_pending"] is True
                        )
                    ):
                        self.info(
                            "Agentic replay barrier stall details: "
                            f"{self._credit_issuer.replay_gate.pending_diagnostics()}"
                        )
                        if self._branch_orchestrator is not None:
                            self.info(
                                "Agentic branch stall details: "
                                f"{self._branch_orchestrator.pending_work_diagnostics()}"
                            )
                        stall_details_reported = True
    ...
    async def _wait_for_sending_complete(
        ...
        try:
            ...
            if drained_normally:
                state = self._generation_drain_snapshot()
                self.notice(
                    "Agentic generation drain complete: "
                    + " ".join(f"{key}={value}" for key, value in state.items())
                )
